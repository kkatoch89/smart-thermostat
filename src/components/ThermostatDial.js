...
class ThermostatDial extends Component {
	...
	render() {
		...
		const min = this.restrictToRange(
			Math.round(
				((actualMinValue - this.props.minValue) / rangeValue) *
					this.props.numTicks
			),
			0,
		...
		const max = this.restrictToRange(
			Math.round(
				((actualMaxValue - this.props.minValue) / rangeValue) *
					this.props.numTicks
			),
			0,
			this.props.numTicks - 1
		...
		const leafDef = [
			...
			.map((point) => _self.mapLeafPoint(point, leafScale))
			.join(' ');
		...
		let degs =
			(tickDegrees * (peggedValue - this.props.minValue)) / rangeValue -
			offsetDegrees;
		...
		const ambientPosition = this.rotatePoint(lblAmbientPosition, degs, [
			radius,
			radius,
		]);
